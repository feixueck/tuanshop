...
<div class="content-box">
      ...
      <div class="content-box-header">
        ...
        <div class="tab-content default-tab" id="tab1">
          ...
          <table>
           <form name="myForm" action="${staticServer}/user/addUserRole" method="post">
            ...
            <tbody>
            	...
            	<#list roleList as role>
	            	 ...
	            	 <#if (role_index+1)%4==0>
	                   <td><input type="checkbox" <#if alist?seq_contains(role.id)>checked="checked"</#if> name="auths" value=${role.id} />${role.name}    </td></tr> <tr>
	                 	<#else>
	                    <td><input type="checkbox" <#if alist?seq_contains(role.id)>checked="checked"</#if> name="auths" value=${role.id} />${role.name}   </td>
	               </#if>
            	 </#list>
            	 </tr>
         	   <tr>
            			<td><input  class="btn5" type="button" onclick="window.open('${staticServer}/user/index')" value="返回列表" /></td>
